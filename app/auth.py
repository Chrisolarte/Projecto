...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM forgotlink WHERE challenge=? and state=? and CURRENT_TIMESTAMP BETWEEN created AND validuntil', (number, utils.F_ACTIVE)#VALIDAR QUE challenge sea igual a number y que su estado este activo
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            user = db.execute(
                'SELECT * FROM user WHERE email =?', (email,)#REVISDAR SI EL CORREO EXISTE EN BD EN (user)
            ).fetchone()
            ...
            if user is not None:
                ...
                db.execute(
                    'UPDATE forgotlink SET state =? WHERE userid =?',#poner estado inactivo en todos los registros que tengan el id del usuario
                    (utils.F_INACTIVE, user['id'])
                )
                db.execute(
                    'INSERT INTO forgotlink (userid,challenge,state) VALUES (?,?,?)',#crear un nuevo registro en tabla forgotlink
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if error is None:
                ...
                return redirect(url_for('inbox.send.html'))
